...
// Time 对象的"-"重载，实现两 Time 对象做减法，没有处理 t2 早于 t1 的情况
...
//查询城市是否存在
bool ALGraph::ifCityExist (const string &city_name) {  // 查询城市是否存在
...
vector<LineNode> ALGraph::getLineNode (const string sc, const string ec, const string amt) {
...
// 输出从起点城市到终点城市的线路
vector<vector<LineNode>> ALGraph::getPathsByCity (const string &sc, const string &ec) {
    ...
    if (it == m.cend()) {
        ...
        vector<vector<LineNode> > result;  // 存放 sc 到 ec 的多条路径
...
// 打印从起点城市到终点城市的所有路径
void ALGraph::printPathsByCity (const string &sc, const string &ec) {
...
void ALGraph::printLeastTransferPath (const string &sc, const string &ec) {
...
} // printLeastTransferPath

// 输出从起点城市到终点城市，总时间最短的线路
void ALGraph::printLeastTimePath (const string &sc, const string &ec) {
...
void ALGraph::printLeastMoneyPath (const string &sc, const string &ec) {
    ...
    while (m_iter != m.cend()) {
        distanced.insert ( {(*m_iter).first.start_city_name, 0x7fffffff} );
        ++m_iter;
    }
    ...
    for (int i=0; i < city_num; ++i) {
        ...
        float min_dist_value = 0x7fffffff;
